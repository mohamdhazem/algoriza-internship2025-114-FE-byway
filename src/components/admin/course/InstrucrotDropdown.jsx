...
export const InstructorDropdown = ({ value, instructorsList, onChange, mode, disabled = false }) => {
    ...
    const handleSelect = (option) => {
        onChange({
            target: { name: "instructorId", value: option.value },
        });
        setIsOpen(false);
    };
    ...
    return (
        <div className="relative w-full">
            ...
            {/* Dropdown */}
            {isOpen && (
                <ul className="absolute mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-10 h-50 overflow-auto">
                    {options.map((option) => (
                        <li
                            key={option.value}
